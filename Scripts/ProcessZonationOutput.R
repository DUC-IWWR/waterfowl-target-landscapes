...
#load rasters from Zonation output and assign unique names
raster.list <- list.files(path = "Output", pattern = "rankmap.tif",full.names = T, recursive = T)
raster.names <- gsub(".*/(.+)/.*", "\\1", raster.list) #extract unique names for each raster
rankmaps <- lapply(raster.list, rast)
for(i in 1:length(rankmaps)) {
  names(rankmaps[[i]]) <- raster.names[[i]]
}

#query out any raw Zonation output that you don't want processed
rankmaps <- rankmaps[c(9,10,11, 15)]

#Reclassify rank rasters into bins
#create directory for reclassified rasters
dir.create("Output/ZonationPostprocessing", showWarnings = F)
dir.create("Output/ZonationPostprocessing/ReclassRasters", showWarnings = F)

#define reclass table and reclassify (select prefered rcl table or create a new one)
# rcl <- matrix(data = c(0.00,0.80,0.85,0.90,0.95,0.80,0.85,0.90,0.95,1.00,1,2,3,4,5), 
#               nrow=5, ncol=3)
rcl <- matrix(data = c(-0.01,0.50,0.60,0.70,0.80,0.90,0.50,0.60,0.70,0.80,0.90,1.0,1,2,3,4,5,6), 
              nrow=6, ncol=3)
rcl.rankmaps <- lapply(rankmaps, 
                 function(x) {
                   return(classify(x = x, rcl = rcl, filename = paste0("Output/ZonationPostprocessing/ReclassRasters/", names(x),"_reclass.tif")))
                 })

#Create various priority areas polygons
#load reclassified Zonation output rasters if needed
rcl.rankmaps <- lapply(list.files("Output/ReclassRasters", pattern = "tif"), rast) 

#Run line below if you want to create polygons only for specific rasters (change pattern as appropriate)
#rasters <- lapply(list.files("Output/ReclassRasters", pattern = "CAZ"), rast)
...
calcPoly <- function(x) {
  ...
  patches = patches(q95, directions = 8)
  patches25 = data.frame(freq(patches))
  patches25 = patches25[patches25$count >=25, ]
  patches25 = patches25$value
  q95[!(patches %in% patches25)] <- NA
  q95 = as.polygons(q95, aggregate = T)
  file = paste0("Output/", names(x),"_q95")
  writeVector(q95, filename = , filetype = "ESRI Shapefile")
  
  #90% quantile
  q90 = x
  q90[which(q90[]<4)] <- NA
  q90[which(q90[]>=4)] <- 1
  #remove isolated patchess of pixels (all patchess with < 25 pixels)
  patches = patches(q90)
  patches25 = data.frame(freq(patches))
  patches25 = patches25[patches25$count >=25, ]
  patches25 = patches25$value
  q90[!(patches %in% patches25)] <- NA
